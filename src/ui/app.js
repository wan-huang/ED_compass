    ...
    return `
      <div class="card care-options-card">
        ...
        ${this.accessOptionsVisible ? `
          ${availabilityAlerts.length ? `<div class="availability-alerts" role="status">${availabilityAlerts.map(alert => `<div class="availability-alert ${alert.severity}"><strong>${alert.severity === 'warning' ? '⏰' : 'ℹ️'} ${t(this.locale, alert.code === 'NO_UPCC_IN_COMMUNITY' ? 'careAvailability' : 'hoursMayVary')}</strong><span>${t(this.locale, alert.code === 'NO_UPCC_IN_COMMUNITY' ? 'noUpccAlert' : 'afterHoursAlert')}</span></div>`).join('')}</div>` : ''}
      ...
      <div class="card feedback-card">
        ...
        <form onsubmit="event.preventDefault(); window.app.submitPatientFeedback({
          ...
          <div class="feedback-grid">
            ...
            <label class="form-group"><span class="form-label">${fr ? 'Principal obstacle' : 'Main access barrier'}</span><select name="accessBarrier" class="form-select"><option value="">${fr ? 'Aucun' : 'None'}</option><option value="transportation">${fr ? 'Transport' : 'Transportation'}</option><option value="childcare">${fr ? 'Garde d’enfants' : 'Childcare'}</option><option value="distance">Distance</option><option value="weather">${fr ? 'Météo ou état des routes' : 'Weather or road conditions'}</option><option value="cost">${fr ? 'Coût' : 'Cost'}</option><option value="language">${fr ? 'Langue' : 'Language'}</option><option value="mobility">${fr ? 'Mobilité ou handicap' : 'Mobility or disability'}</option><option value="trust">${fr ? 'Sécurité culturelle ou confiance' : 'Cultural safety or trust'}</option></select></label>
